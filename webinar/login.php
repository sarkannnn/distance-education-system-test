...
<div class="max-w-md mx-auto py-20">
    <div class="bg-[#0a1f44] rounded-[2.5rem] border border-white/5 p-10 shadow-2xl">
        <div class="text-center mb-10">
            <div
                class="inline-flex items-center justify-center w-20 h-20 rounded-3xl <?php echo $role === 'teacher' ? 'bg-slate-700' : 'bg-blue-600'; ?> mb-6 shadow-2xl">
                <i data-lucide="<?php echo $role === 'teacher' ? 'user-circle' : 'graduation-cap'; ?>"
                    class="w-10 h-10 text-white"></i>
            </div>
            <h2 class="text-2xl font-bold mb-1 uppercase tracking-tight">
                <?php echo $role === 'teacher' ? 'Mühazirəçi' : 'İştirakçı'; ?> Girişi
            </h2>
        ...
        <?php if ($error): ?>
            <div
                class="bg-rose-500/10 border border-rose-500/20 text-rose-500 p-4 rounded-2xl mb-8 text-sm font-medium text-center">
                <?php echo e($error); ?>
            </div>
        <?php endif; ?>
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <button type="button" onclick="toggleLoginPassword()"
                        class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-emerald-400 transition-colors cursor-pointer p-1">
        ...
        <div class="mt-8 pt-8 border-t border-white/5 text-center">
            <a href="index.php"
                class="text-xs font-bold text-white/30 hover:text-white transition-colors uppercase tracking-widest flex items-center justify-center gap-2">
