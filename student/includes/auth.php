    ...
    private function syncToLocalDb(array $user): void
    {
        try {
            ...
            if ($existing) {
                ...
                $db->query(
                    "INSERT INTO users (student_id, first_name, last_name, email, role, is_active, created_at, updated_at, password)
                     VALUES (?, ?, ?, ?, 'student', 1, NOW(), NOW(), ?)",
                    [$studentId, $user['first_name'], $user['last_name'], $user['email'], password_hash(bin2hex(random_bytes(10)), PASSWORD_DEFAULT)]
                );
            }
        } catch (Exception $e) {
