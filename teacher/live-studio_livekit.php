        ...
        <div class="studio-center"
            ...
            <div id="mainControlsBar"
                ...
                <div id="mainControlsInner"
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                        <button id="btnModeNormal" onclick="setQualityMode('normal')" class="control-btn active-green"
                            style="width: 50px; border-radius: 50%;" title="Standart Keyfiyyət">
                            <i data-lucide="zap" style="width: 22px; height: 22px;"></i>
                        </button>
                        <span
                            style="font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px;">NORMAL</span>
                    </div>

                    <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                        <button id="btnModeEco" onclick="setQualityMode('eco')" class="control-btn"
                            style="width: 50px; border-radius: 50%;" title="Eco Mode (Zəif İnternet)">
                            <i data-lucide="leaf" style="width: 22px; height: 22px;"></i>
                        </button>
                        <span
                            style="font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px;">ECO</span>
                    </div>
...
<div id="screenRequestModal"
    style="display: none; position: fixed; inset: 0; background: rgba(2,6,23,0.85); z-index: 10000; backdrop-filter: blur(10px); justify-content: center; align-items: center;">
    <div
        ...
        <div
            style="width: 80px; height: 80px; background: rgba(16, 185, 129, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 25px;">
            <i data-lucide="monitor" style="width: 40px; height: 40px; color: #10b981;"></i>
        </div>
        <h3 style="margin: 0 0 10px 0; font-size: 20px; font-weight: 850; color: white;">Ekran Paylaşımı</h3>
        <p style="color: #94a3b8; font-size: 14px; margin-bottom: 30px; line-height: 1.6;"><b><span
                    id="screenRequesterName" style="color: #10b981;"></span></b> ekranını paylaşmaq üçün icazə istəyir.
        </p>
        <div style="display: flex; gap: 15px;">
            <button id="screenRejectBtn"
                style="flex: 1; background: rgba(255,255,255,0.05); color: white; border: 1px solid rgba(255,255,255,0.1); padding: 14px; border-radius: 12px; font-weight: 700; cursor: pointer;">RƏDD
                ET</button>
            <button id="screenApproveBtn"
                style="flex: 1; background: #10b981; color: white; border: none; padding: 14px; border-radius: 12px; font-weight: 700; cursor: pointer; box-shadow: 0 10px 20px rgba(16, 185, 129, 0.3);">İCAZƏ
                VER</button>
        </div>
    </div>
</div>
...
<div id="whiteboardOverlay">
    ...
    <div
        style="position: absolute; top: 15px; left: 15px; right: 15px; display: flex; justify-content: space-between; align-items: flex-start; z-index: 2010; pointer-events: none;">

        <!-- Info Badge (Top Left) -->
        <div
            style="background: rgba(15, 23, 42, 0.65); color: white; border: 1px solid rgba(255, 255, 255, 0.1); padding: 8px 16px; border-radius: 100px; font-weight: 600; font-size: 11px; letter-spacing: 0.5px; display: flex; align-items: center; gap: 8px; backdrop-filter: blur(10px); box-shadow: 0 4px 15px rgba(0,0,0,0.1); pointer-events: auto;">
            <div
                style="width: 8px; height: 8px; background: #ef4444; border-radius: 50%; animation: blink 1s infinite; box-shadow: 0 0 8px rgba(239, 68, 68, 0.8);">
            </div>
...
<script>
    ...
    function sendChatMessage() {
        ...
        if (!room || room.state !== 'connected') {
            alert("Canlı yayım qoşulu deyil!");
            return;
        }
        ...
        if (privateTarget) {
            msgObj.isPrivate = true;
            msgObj.sender = 'Müəllim (Özəl)';
            // In LiveKit migration, privateTarget.peer holds the participant identity
            broadcastData(msgObj, privateTarget.peer);
    ...
    async function broadcastData(data, targetIdentity = null) {
    ...
    function showMicRequestModal(senderName, pId) {
    ...
    async function init() {
        try {
            ...
            function showWhiteboardRequestModal(name, pId) {
            ...
            function showScreenShareRequestModal(name, pId) {
                document.getElementById('screenRequesterName').innerText = name;
                const modal = document.getElementById('screenRequestModal');
                modal.style.display = 'flex';
            ...
            async function startLiveKit() {
                try {
                    ...
                    LOG("❌ LiveKit Xətası: " + e.message, "#ef4444");
            ...
            function setupRoomEvents() {
                room
                    .on(LivekitClient.RoomEvent.ParticipantConnected, (p) => LOG(`👥 ${p.name || p.identity} qoşuldu.`))
                    .on(LivekitClient.RoomEvent.ParticipantDisconnected, (p) => {
                        LOG(`👥 ${p.name || p.identity} ayrıldı.`, "#f59e0b");
                        removeStudentVideo(p.identity);
                    })
                    .on(LivekitClient.RoomEvent.TrackSubscribed, (track, publication, participant) => {
                        if (track.kind === 'video') {
                            const name = participant.name || participant.identity;
                            // Let LiveKit attach internally to handle plumbing
                            track.attach();
                            ...
                            if (publication.source === LivekitClient.Track.Source.ScreenShare) {
                                startStudentSpotlight(participant.identity, stream, name, track);
                            } else {
                                addStudentVideo(participant.identity, stream, name, participant.identity, track);
                            }
                        }
                        if (track.kind === 'audio') {
                            track.attach();
                    ...
                    .on(LivekitClient.RoomEvent.TrackUnsubscribed, (track, publication, participant) => {
                        if (track.kind === 'video') {
                            if (publication.source === LivekitClient.Track.Source.ScreenShare) {
                                ...
                                removeStudentVideo(participant.identity);
                            }
                        }
                    })
                // Redundant event registration for maximum compatibility
                const dataHandler = (payload, participant, kind, topic) => {
                    const decoder = new TextDecoder();
                    try {
                        const d = JSON.parse(decoder.decode(payload));
                        LOG("📡 Siqnal alındı: " + d.type, "#94a3b8");
                        console.log("📥 Data Received:", d, "from", participant?.identity, "topic:", topic);
                ...
                if (LivekitClient.RoomEvent.DataReceived) {
                    room.on(LivekitClient.RoomEvent.DataReceived, dataHandler);
                }
            }
            ...
            async function publishMedia() {
                ...
                try {
                    ...
                    console.log("🎥 Publishing tracks to LiveKit:", tracks.map(t => t.kind));

                    let videoPublished = false;
                    for (const track of tracks) {
                        track.enabled = true;
                        if (track.kind === 'video' && !videoPublished) {
                            const localVideo = new LivekitClient.LocalVideoTrack(track);
                            await room.localParticipant.publishTrack(localVideo, {
                                source: LivekitClient.Track.Source.Camera,
                                simulcast: false, // Disable simulcast for composited canvas streams (stability)
                                videoEncoding: {
                                    maxBitrate: 1000000,
                                    maxFramerate: 20
                                }
                    ...
                    LOG("🎥 Media yayımlanır.");
                } catch (e) {
                    LOG("❌ Media yayım xətası: " + e.message, "#ef4444");
            ...
            function handleIncomingData(d, participant) {
                ...
                const sender = d.sender || (participant ? (participant.name || participant.identity) : "Tələbə");
                const pId = participant ? participant.identity : (d.peerId || d.senderId);

                if (d.type === 'chat' || d.type === 'file') {
                    const isPrivate = d.isPrivate || false;
                    const color = isPrivate ? '#3b82f6' : '#10b981';
                    if (d.type === 'chat') {
                        appendChatMessage(sender, d.message, color);
                    } else {
                        appendFileMessage(sender, d.fileName, d.fileData, color);
                    }
                } else if (d.type === 'mic_request') {
                    LOG("🎤 Mikrofon istəyi: " + sender, "#3b82f6");
                    showMicRequestModal(sender, pId);
                } else if (d.type === 'whiteboard_request') {
                    LOG("✏️ Lövhə istəyi: " + sender, "#3b82f6");
                    showWhiteboardRequestModal(sender, pId);
                } else if (d.type === 'whiteboard_ended') {
                    LOG("✏️ Lövhə bitdi: " + sender, "#f59e0b");
                    if (pId === spotlightPeerId) stopStudentSpotlight();
                } else if (d.type === 'screen_share_request') {
                    LOG("🖥️ Ekran istəyi: " + sender, "#3b82f6");
                    showScreenShareRequestModal(sender, pId);
                } else if (d.type === 'screen_share_ended') {
                    LOG("🖥️ Ekran bitdi: " + sender, "#f59e0b");
                    if (pId === spotlightPeerId) stopStudentSpotlight();
                }
            }
    ...
    function startCanvasCompositing() {
        ...
        // Initial draw to prevent black screen on subscription
        ctx.fillStyle = '#0f172a';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.fillStyle = 'white';
        ctx.font = '24px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('Yayım Hazırlanır...', canvas.width / 2, canvas.height / 2);

        const canvasStream = canvas.captureStream(20);
        LOG("🎨 Canvas Compositor aktivdir.");
    ...
    function drawToCanvas() {
        ...
        if (isStudentSpotlight && studentScreenVidElement && studentScreenVidElement.readyState >= 2) {
            ...
            // Draw TEACHER'S camera in corner during student spotlight
            const teacherCam = document.getElementById('camSource');
            if (teacherCam && teacherCam.readyState >= 2) {
                const cW = teacherCam.videoWidth;
                const cH = teacherCam.videoHeight;
                const cRatio = cW / cH;
                const camDrawW = 220;
                ...
                ctx.drawImage(teacherCam, cx, cy, camDrawW, camDrawH);
                ctx.restore();
    ...
    function addStudentVideo(peerId, stream, name, studentId = null, track = null) {
        console.log(`📹 addStudentVideo called for ${name} (${peerId}). Track kind:`, track ? track.kind : 'null');
        ...
        if (existingCard) {
            ...
            if (vid) {
                if (track) track.attach(vid);
                else vid.srcObject = stream;
        ...
        badge.innerHTML = `<div style="width:6px; height:6px; background:#22c55e; border-radius:50%;"></div><span>${name}</span>`;

        // Action Menu Button
        const menuBtn = document.createElement('button');
        menuBtn.innerHTML = '<i data-lucide="more-vertical" style="width:14px; height:14px; color:white;"></i>';
        menuBtn.style.cssText = "position:absolute; top:8px; right:8px; width:28px; height:28px; background:rgba(0,0,0,0.5); border:none; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; z-index:10; transition:0.2s;";

        // Moderation Dropdown
        const dropdown = document.createElement('div');
        dropdown.className = 'student-dropdown';
        dropdown.id = 'dropdown-' + peerId;
        dropdown.style.cssText = "display:none; position:fixed; background:#1e293b; border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:6px; z-index:99999; box-shadow:0 10px 25px rgba(0,0,0,0.5); min-width:160px; backdrop-filter:blur(10px);";

        const createMenuItem = (icon, text, color, onClick) => {
            const btn = document.createElement('button');
            btn.innerHTML = `<i data-lucide="${icon}" style="width:14px; height:14px;"></i> ${text}`;
            btn.style.cssText = `width:100%; text-align:left; background:none; border:none; color:${color}; padding:10px 12px; border-radius:6px; font-size:12px; cursor:pointer; display:flex; align-items:center; gap:8px; transition:0.2s;`;
        ...
        const muteItem = createMenuItem('mic-off', 'Səsi kəs', '#f59e0b', () => muteStudent(peerId, name));
        const spotlightItem = createMenuItem('maximize', 'Böyüt (Spotlight)', '#3b82f6', () => focusStudent(stream, name));
        const chatItem = createMenuItem('message-square', 'Özəl Mesaj', '#8b5cf6', () => setPrivateTarget(peerId, name));
        const kickItem = createMenuItem('user-x', 'Dərsdən çıxar', '#ef4444', () => kickStudent(peerId, name, studentId));

        dropdown.append(muteItem, spotlightItem, chatItem, kickItem);
        document.body.appendChild(dropdown);

        menuBtn.onclick = (e) => {
            e.stopPropagation();
            const isOpen = dropdown.style.display === 'block';
            document.querySelectorAll('.student-dropdown').forEach(d => d.style.display = 'none');
        ...
        document.getElementById('activeVideoCount').innerText = grid.children.length;
    }
        ...
        if (isScreenSharing) {
            ...
            if (screenStream) {
                ...
                try {
                    ...
                    console.error("Unpublish screen error", e);
                }
            }
        } else {
            try {
    ...
    function startStudentSpotlight(peerId, screenStream, name, track = null) {
        ...
        if (!hiddenContainer) {
            ...
            // DO NOT USE display:none as it throttles video updates in some browsers
        ...
        if (track) track.attach(studentScreenVidElement);
        else studentScreenVidElement.srcObject = screenStream;
    ...
    function replaceTrack(newTrack) {
        if (!room || room.state !== 'connected') return;
        // In LiveKit, we use unpublish/publish or replaceTrack if available in localParticipant
    }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (type === 'bulk') {
                        ...
                        // Send to specific student via LiveKit
                        broadcastData(msgObj, targetId);
    ...
    function trackAttendance(type) {
        ...
        if (typeof room !== 'undefined' && room && room.localParticipant) params.peer_id = room.localParticipant.identity;
        if (type === 'leave') {
    ...
    setInterval(() => {
        if (room && room.state === 'connected') trackAttendance('heartbeat');
    }, 30000);
