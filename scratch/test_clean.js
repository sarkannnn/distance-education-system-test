    ...
    function createDummyStream(realAudioTrack = null) {
        ...
        ctx.fillText("Tələbə: null", 320, 350);
    ...
    function swapVideoSources() {
        ...
        if (isViewSwapped) {
            ...
            mainLabel.innerHTML = '<div style="width: 8px; height: 8px; background: #3b82f6; border-radius: 50%; box-shadow: 0 0 8px #3b82f6;"></div>Müəllim: null';
    ...
    async function startActualScreenShare() {
        try {
            ...
            if (room && room.state === 'connected') {
                await room.localParticipant.publishTrack(screenTrack, { source: LivekitClient.Track.Source.ScreenShare });
            }
    ...
    async function stopScreenShare() {
        ...
        if (camVideoTrack) {
            ...
            document.getElementById('localPreview').srcObject = localMediaStream;
            // Republish normal camera if needed, though LiveKit might need explicit unpublish/publish
        }
